...
def parse_args():
    ...
    # model
    parser.add_argument('--arch', '-a', metavar='ARCH', default='DUKAN')
...
def train(config, train_loader, model, criterion, optimizer):
    ...
    for input, target, _ in train_loader:
        ...
        if config['deep_supervision']:
            ...
            output, S2, S3 = model(input)
            if output.shape[2:] != target.shape[2:]:  # 如果尺寸不一致
                output = F.interpolate(output, size=target.shape[2:], mode='bilinear', align_corners=True)
            if S2.shape[2:] != target.shape[2:]:
                S2 = F.interpolate(S2, size=target.shape[2:], mode='bilinear', align_corners=True)
            if S3.shape[2:] != target.shape[2:]:
                S3 = F.interpolate(S3, size=target.shape[2:], mode='bilinear', align_corners=True)
            loss = 0.6 * criterion(output, target) + 0.2 * criterion(S2, target) + 0.2 * criterion(S3, target)
